...
def promote(message):
    ...
    if target_name not in slack.users:
        pb_send(channel, "Player \"%s\" not found." % target_name)
        return
    admins.append(target_name)
    pb_send(channel, "Promoted %s to admin" % target_name)
    echo("User %s promoted %s" % (slack.get_user_name(message['user']), target_name))
...
def demote(message):
    ...
    words = message['text'].split()[2:]
    user_name = '_'.join(words)
    if target_name not in slack.users.keys() + admins:
        pb_send(channel, "Player \"%s\" not found." % target_name)
        return
    admins.remove(target_name)
    pb_send(channel, "Demoted %s to user" % target_name)
    echo("User %s demoted %s" % (slack.get_user_name(message['user']), target_name))
